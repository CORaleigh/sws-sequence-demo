...
const viewLoaded = async (view) => {
  ...
  routePicker.watch("values", (values) => routeChanged(values, view, layer, sequencePicker));
  sequencePicker.on("animate", () => animated(view, layer, layerView,  sequencePicker.values[0], routePicker.values[0]));  
}
...
const routeChanged = async (values, view, layer, valuePicker) => {
  if (values.length) {
    ...
    const max = (await getMaxSequence(layer, routePicker)) / 10;
    valuePicker.component.max = max;
    valuePicker.component.steps = getSteps(max);
    valuePicker.component.labels = getLabels(max);
...
const animated = async (view, layer, layerView, sequence, route) => {
  const query = layer.createQuery();
